 ...
 *
 * Major modification:
 * The lines 541 and 542 have been moved from `prepareAssetForPlayback` to
 * `loadLocalMediaFromURL:url`. The `getVideoWidth`, `getVideoHeight` and
 * `getLength` methods have been modified accordingly, so that the video
 * resolution and length are available immediately after the video has been
 * loaded, and until it is unloaded.
 *
 * Other modifications:
...
//    (void)AudioSessionSetActive(true);
    ...
    if (NOT_READY != mediaState && ERROR != mediaState) {
        ...
        mediaURL = [[NSURL alloc] initFileURLWithPath:path];
...
// Play the asset
- (BOOL)playFromPosition:(float)seekPosition
{
    BOOL ret = NO;
        ...
        switch (status) {
            ...
            case AVPlayerItemStatusReadyToPlay:
                ...
                if (YES == playImmediately) {
                    [self playFromPosition:VIDEO_PLAYBACK_CURRENT_POSITION];
                }
...
- (BOOL)loadLocalMediaFromURL:(NSURL*)url
{
    ...
    // Get video properties *as soon as possible*
